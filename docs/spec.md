...
提出実行
  → Gemini API 判定
  → ai_risk_score 算出
  → Google Drive アップロード（ファイル名: 団体名-企画名-提出先-(スコア%).拡張子）
  → 自動承認判定:
...
- ヘッダー：「生駒祭 情宣AI」「2026」
- メニュー項目：事前チェック、提出、提出履歴、（管理者のみ）ダッシュボード、ルール管理、マスタ管理、設定
- フッター：バージョン表示（v1.0.0）
- 背景色：ダークテーマ（#1a1a2e）
...
4. リトライ:「ホームに戻る」ボタンで事前チェック画面に遷移可能
...
2. `josenai_profiles` に upsert（情宣固有データ用: 事前チェック回数カウント等）
...
情宣システム固有のユーザーデータ。事前チェック利用制限を管理する。

| カラム名 | 型 | Null | 説明 |
|---------|-----|------|------|
| id | UUID | NOT NULL | Primary Key (gen_random_uuid()) |
| user_id | UUID | NOT NULL | Foreign Key → auth.users(id)（UNIQUE） |
| sandbox_count_today | INTEGER | NOT NULL | 本日の事前チェック使用回数（デフォルト: 0） |
| sandbox_count_date | DATE | NULL | 事前チェックカウント日付（JST 0:00 リセット） |
| created_at | TIMESTAMPTZ | NOT NULL | 作成日時 |
| updated_at | TIMESTAMPTZ | NOT NULL | 更新日時 |
...
| sandbox_daily_limit | 事前チェック1日上限 | 3 |
...
auth.users (Supabase共有認証)
    ...
    │
    ├── 1:1 → josenai_profiles (情宣固有: 事前チェック制限)
    │
...
- Q: 事前チェック画面に「提出先」選択 → A: 追加する（提出種別により判定基準が異なる場合に対応）
...
- Q: 事前チェック制限リセット → A: JST 0:00（日本時間の深夜）
- Q: 審査結果の通知方法 → A: アプリ内通知のみ
- Q: 同一組み合わせでの複数回提出 → A: 可能（新規提出として追加、過去の提出も履歴に残る）
- Q: 提出後の削除・編集 → A: pending状態のみ削除可能（審査開始後は不可）
- Q: Google Driveフォルダ構成 → A: 提出種別・団体別フォルダ（/生駒祭2026/企画物/{団体}/ or /生駒祭2026/SNS/{団体}/）。ファイル名は「団体名-企画名-提出先-(スコア%).拡張子」形式
